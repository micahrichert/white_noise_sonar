...
constexpr double MAX_DISTANCE = 1.5;//1.5;//1.5; // m
constexpr int SAMPLE_LAG = 0;  // must be a multiple of HIST_TYPE_BITS
constexpr double MAX_SPEED = 1.2;  // m/s
constexpr double MIN_SPEED = 0.0;  // m/s
constexpr double SPEED_BANDWIDTH = 0.1;  // m/s, when a large target is stationary what is the fastest single motion channel that still detects it reasonably
